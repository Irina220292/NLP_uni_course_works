...
def main(argv):
    ...
    model.compile(optimizer='nadam', loss='categorical_crossentropy', metrics=['accuracy'])  # am Besten: nadam, 6 Epochs: 56.02 / adamax, 27 Epochs: 56.10
    model.fit(train_matrix, train_labels, validation_data=(dev_matrix, dev_labels), epochs=6)
...
def tokens_and_labels(filename):
    ...
    with open(filename, encoding='utf-8') as f:
        for line in f:
            ...
            tokens = tokens.replace("<SUBJ>", "<SUBJ> " + subject_name)
            tokens = tokens.replace("<OBJ>", "<OBJ> " + object_name)
